...
method.init = function() {
  ...
  // console.debug("CCI started with:\nup:\t", this.uplevel, "\ndown:\t", this.downlevel, "\npersistence:\t", this.persisted);
...
method.log = function(candle) {
    ...
    if (typeof(cci.result) == 'boolean') {
        console.debug('Insufficient data available. Age: ', cci.size, ' of ', cci.maxSize);
        return;
    }

    console.debug('calculated CCI properties for candle:');
    console.debug('\t', 'Price:\t\t', candle.close.toFixed(8));
    console.debug('\t', 'CCI tp:\t', cci.tp.toFixed(8));
    console.debug('\t', 'CCI tp/n:\t', cci.avgtp.toFixed(8));
    console.debug('\t', 'CCI md:\t', cci.mean.toFixed(8));
    if (typeof(cci.result) == 'boolean' )
        console.debug('\t In sufficient data available.');
    else
        console.debug('\t', 'CCI:\t\t', cci.result.toFixed(2));
}
...
method.check = function(candle) {
    ...
    console.debug("Trend: ", this.trend.direction, " for ", this.trend.duration);
}
